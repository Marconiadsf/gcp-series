# Google Cloud Run Experiments
	...
	This repository contains a series of experiments and demo applications to explore deployment
	of aplications to Google Cloud Run, using docker containers, and unsing docker containers for 
	local development and testing. 

## Description of Demo Applications
### DemoApp-01 : Local deployment example
	This is a simple demo application to illustrate local deployment to Google Cloud Run using the `gcloud
	command-line tool.`.
### DemoApp-02 : GitHub Actions CI/CD Pipeline
	This demo application illustrates how to set up a CI/CD pipeline using GitHub Actions to automate
	the deployment of the same application to Google Cloud Run whenever changes are pushed to the 
...
## How to Use This Repository
	...
	Use VSCode to start a new terminal:
		...
		It may start in something like: C:\Users\YourName\
	...
	Now cd into the gcp-series folder for the unziped files:
	```
	cd gcp-series
	````
	...
	```
	This will build the development container image with  python:3.10-slim, git, gcloud SDK, and toher tools, copy
	...
	Keep the terminal open after runing the container, you are now inside the container.
	If you type ls you may see the folders DemoApp-01, DemoApp-02, the files: Dockerfile.dev, Dockerfile, and requirements.txt

	If you keeped it only inside the container, you can open the folder in VSCode using the "Remote - Containers" extension:

		Click in "Remmote-Containers" icon in the left sidebar
		...
		In the menu select the folder you want to open, whithing the container, e.g., /workspace.
		...
		#### 1. copy the files from your host to the container using docker cp command:
			Open a new terminal in your host machine (not inside the container) and navigate to the gcp-series folder you created
			before.
			Type the command:
			```
	...
	Either way, you can now navigate to each demo application folder and follow the specific instructions in their README.mk files.
